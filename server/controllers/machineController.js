...
const createMachine = async (req, res) => {
  ...
  if (mId.length < 6) {
    return res.status(400).json({
      error: "Machine ID must include atleast 6 characters. Eg: XXX000",
    });
  }
...
const getMachinesByFId = async (req, res) => {
  const sanitizedFactoryId = xss(req.params.mFactory);
  const machine = await Machine.findByFId(sanitizedFactoryId);
...
const updateMachine = async (req, res) => {
  ...
  if (mId.length < 5) {
    return res.status(400).json({
      error: "Machine ID must include atleast 5 characters. Eg: XXX00",
    });
  }
